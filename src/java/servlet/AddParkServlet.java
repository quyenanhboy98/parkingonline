 ...
 * @author Yuu
 */
@WebServlet(name = "AddParkServlet", urlPatterns = {"/AddParkServlet"})
public class AddParkServlet extends HttpServlet {

    private final String page = "addpark.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (parkID == null || slotStr == null || parkName == null || parkAddress == null || priceStr == null
                ...
                isError = true;
                error.setCreateError("Thêm thất bại. Có lỗi xảy ra. Vui lòng thử lại.");
                return;
            }

            if (parkID.isEmpty()) {
                isError = true;
                error.setParkIDIsEmtpy("ID bãi xe không thể bỏ trống.");
            }
            if (parkName.isEmpty()) {
                isError = true;
                error.setParkNameIsEmpty("Tên bãi xe không thể bỏ trống.");
            }
            if (parkAddress.isEmpty()) {
                isError = true;
                error.setParkAddressIsEmpty("Địa chỉ bãi xe không thể bỏ trống.");
            }
            ...
            if (slotStr.isEmpty() || !slotStr.matches("^\\d+$")) {
                isError = true;
                error.setParkSpaceIsWrongFormat("Không gian bãi phải là số nguyên dương.");
            } else {
                slot = Integer.parseInt(slotStr);
                if (slot <= 0) {
                    isError = true;
                    error.setParkSpaceIsWrongFormat("");
                }
            }
            ...
            if (!isError) {
                ParkDTO park = new ParkDTO(parkID, parkName, parkAddress, slot, price, lat, lng);
                if (!addPark(park)) {
                    isError = true;
                }
            }
            ...
            if (msg.contains("Duplicate")) {
                error.setParkIDIsExisted("Bãi xe " + parkID + " đã được đăng ký.");
            } else {
                log(msg);
            }
        } finally {
            if (isError) {
                ...
                request.setAttribute("SUCCESS", "Bãi xe " + parkID + " tạo thành công.");
            }
    ...
    private boolean addPark(ParkDTO user) throws SQLException, ClassNotFoundException {
        ParkDAO dao = new ParkDAO();
        return dao.addPark(user);
    }
}
